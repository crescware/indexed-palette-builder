...
2. **Extract integer part** of each value as anchor (e.g., `0.6809999999999999` * 100 → `68`)
...
culori's `Oklch` type defines hue as optional: `h?: number`. Achromatic colors (white, black, grays) return `undefined` for hue.

For `OklchBig`, hue will be required `h: Big`. When parsing achromatic colors, store `Big(0)` as a placeholder value. The extraction and formatting logic must handle this case.

#### Hex Input Path

`generate-palette-from-hex.ts` converts hex to `Oklch` using culori's converter. This path has no original OKLCH string to extract precise values from.

Options:
1. Create adapter function `oklchToOklchBig(oklch: Oklch): OklchBig` that converts `number` to `Big`
2. Accept that hex-derived values may have minor floating-point variance
3. Document that OKLCH string input preserves precision while hex input does not

#### PaletteStep Type Propagation

The `PaletteStep` type in `generate-palette.ts` includes `oklch: Oklch`. When `OklchBig` is adopted, this must change to `oklch: OklchBig`, affecting `color-palette.tsx` and `css-export.tsx`.
...
The following formats are candidates for future implementation:

#### HEXA (`#rrggbbaa`)

8-digit hexadecimal format with alpha channel support.

| Aspect | Details |
|--------|---------|
| Example | `#ff0000ff` (fully opaque red) |
| Use case | When hex format is preferred but transparency is needed |
| Browser support | Supported in all modern browsers since 2017 |
| Precision | Limited to 256 levels per channel (8-bit) |

**Considerations:**
- Alpha values map to 00-ff range, which may cause rounding artifacts for fractional alpha
- Some developers prefer the shorter `#rgba` 4-digit shorthand when possible
- Legacy tools may not recognize 8-digit hex

#### RGB (`rgb(r, g, b)`)

Functional notation using decimal values 0-255.

| Aspect | Details |
|--------|---------|
| Example | `rgb(255, 0, 0)` |
| Use case | When numeric RGB values are preferred over hex |
| Browser support | Universal |
| Precision | Limited to 256 levels per channel (8-bit) |

**Considerations:**
- Modern CSS allows space-separated syntax: `rgb(255 0 0)`
- Percentage values also valid: `rgb(100%, 0%, 0%)`
- Decision needed: which syntax variant to output
...
| Use case | When RGB format with transparency is needed |
| Browser support | Universal |
| Precision | Alpha accepts decimal values 0-1 |

**Considerations:**
- Modern CSS merged `rgb()` and `rgba()` - both now accept alpha
- Decision needed: use legacy `rgba()` or modern `rgb(r g b / a)` syntax
- The slash syntax aligns with OKLCH: `oklch(l c h / a)`

### Format Selection Logic

When multiple formats are available, the application may need to determine:

1. **Default format**: Which format to show when user has no preference
2. **Recommended format**: Whether to suggest OKLCH for perceptual uniformity
3. **Format grouping**: Whether to group legacy (hex, rgb) vs modern (oklch) formats in the dropdown

### Alpha Channel Handling

Current implementation does not preserve or display alpha channels. Future work must address:

1. **Input parsing**: Detect and preserve alpha from input colors
2. **Palette generation**: How alpha propagates through shade calculations
3. **Format switching**: Converting alpha between formats (hex 00-ff vs decimal 0-1)
4. **UI indication**: Whether to show alpha in tooltips/output when present
