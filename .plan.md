...
- [ ] **Step 10**: Create state type
- [ ] **Step 20**: Add storage key
- [ ] **Step 30**: Create context provider
- [ ] **Step 35**: Create hook
- [ ] **Step 40**: Register provider
- [ ] **Step 50**: Update `color-format-select.tsx` to use context (replace local signal)
- [ ] **Step 70**: Update `settings-popup.tsx` to call `resetColorFormat()`
...
| File | Purpose |
|------|---------|
| `src/models/color-format-state.ts` | State and format type definitions |
| `src/contexts/color-format/color-format-context.tsx` | Context provider |
| `src/contexts/color-format/use-color-format.ts` | Custom hook |
...
| File | Change |
|------|--------|
| `src/models/storage/storage.ts` | Add `colorFormat` storage key |
| `src/app.tsx` | Add `ColorFormatProvider` to hierarchy |
| ~~`src/components/header.tsx`~~ | ~~Add `ColorFormatSelect` component~~ (DONE) |
| `src/components/color-format-select.tsx` | Replace local signal with context |
| `src/components/settings-popup.tsx` | Add reset logic for color format |
...
- Updating color-palette.tsx tooltips to show OKLCH
- Updating css-export.tsx to generate OKLCH CSS variables
- OKLCH formatting utility function
