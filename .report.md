...
### culori Library Behavior: Hue Property

The `h` property in culori's OKLCH representation follows a specific pattern for achromatic colors:

| Source Function | Input Type | Achromatic Color Result | Chromatic Color Result |
|-----------------|------------|------------------------|------------------------|
| `oklch(hex)` | Hex string | `h` property does NOT exist | `h` exists with value |
| `parse("oklch(... h)")` | OKLCH with explicit hue | `h` exists with value | `h` exists with value |
| `parse("oklch(... none)")` | OKLCH with `none` keyword | `h` property does NOT exist | N/A |

Examples tested:
- `oklch("#808080")` returns `{ mode: "oklch", l: 0.599..., c: 0 }` (no `h` key)
- `parse("oklch(50% 0 0)")` returns `{ mode: "oklch", l: 0.5, c: 0, h: 0 }` (`h` key exists)
- `parse("oklch(50% 0 none)")` returns `{ mode: "oklch", l: 0.5, c: 0 }` (no `h` key)

Both `toOklchBig` and `parseOklchWithPrecision` now use spread patterns to preserve this behavior:
```typescript
